...
-- Match each invite to the inviter's first workspace (by created_at).
-- Invites without a matching workspace are skipped (inviter never joined one).
...
-- Skip if this email is already invited to that workspace
WHERE NOT EXISTS (
    SELECT 1
    FROM public.workspace_invites wi
    WHERE wi.workspace_id = wm.workspace_id
      AND wi.email        = ti.email
);
