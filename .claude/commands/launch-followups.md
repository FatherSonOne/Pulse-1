...
| `delete-test-111` | done | P0 | 2026-05-31 | PASSED after fixing the erasure RPC: it was 500ing (5 bugs — FK order, dead team_invites, vox_drops/brainstorm/relationships wrong cols, 10 text-vs-uuid ids, last-owner trigger). Throwaway acct fully erased (auth.users/pulse_users/user_profiles/messages/workspace all 0; audit-logged). Also closed a billing leak: org-delete + erasure now BLOCK while a live Stripe sub exists (1a21f1e) — cancel via portal first |
...
**Resume Pointer:** next unblocked item is `keystore-backup` (P0). Run `/launch-followups` to start there, or `/launch-followups <key>` to jump.
...
- **2026-05-31** — `live-smoke-99` done, but only after discovering the item itself was written against a flow that never existed. Operator saw the Slack settings card expose an `xoxb` **bot-token paste field** (not the "Connect Slack" OAuth button the item assumed) and asked whether work had been lost to destructive git behavior. Investigation (committed as `docs/INTEGRATIONS_BUILT_VS_STUB_AUDIT_2026-05-31.md`): **no work lost** — `git log --all -S "api/slack/callback"` and `-S "ZapierIntegration"` both return 0 real commits; Pulse's Slack integration has always been a bring-your-own-bot-token proxy, Gmail piggybacks Google sign-in, and Zapier was never built (policy-toggle row only). Rewrote the item to the real architecture (commit `3811374`), then hit a genuine code bug mid-smoke: the card documented 7 scopes but `slackService.getChannels()` requests `mpim` group DMs, needing an 8th scope `mpim:read` → `missing_scope`. Fixed the card + PREP docs to list all 8 (commit `0db1c5b`). **Both smokes then PASSED from production:** Slack bot-token pull connected (4 channels) and synced a real message with a resolved sender name ("Frank Messana" via `users.info`), proving the full UI→Render `/api/slack/proxy`→Slack chain incl. `users:read`; Gmail Google-piggyback pull fetched 10 real emails via `/api/gmail/proxy`. Backend health 200 throughout. **Two real follow-ups surfaced (NOT blockers, not fixed):** (1) "Connected to undefined" — `slackService` returns `workspaceName` but the card reads `slackStatus.workspace` (field-name mismatch); (2) synced Slack/SMS messages have **no production reader** — `UnifiedInbox.tsx` is only rendered inside the unrouted `MultiModalDemo`, so the data lands nowhere users can see it (truth-in-product / #104 territory). Slack channel list also renders raw user IDs for DM (`im`) entries that lack a `.name`. 3 × P0 remain.
