...
| **P9** Geofences drawer + ring overlay | `883a911` | `horizon/GeofencesDrawer.tsx` (radius edit/remove via `setPlaceGeofence` + recent `geofence_events` w/ "mark reviewed" = first producer of dead `surfaced`/`surfaced_at` + rings toggle + honest "requires Live ON" banner + `useDialogA11y`); `provider/MapLibreGeofenceRings.tsx` (coral equirectangular ring per geofenced place, mirrors `MapLibreAtlasHalos`); `geofenceService` additive `listRecentGeofenceEvents`/`markGeofenceEventSurfaced`; all wiring `mapHorizonOn`-gated. No migration (cols + UPDATE RLS verified live) | tsc-clean (all touched files) + **20 map tests** + **adversarial review (3 lenses, all clean)** — NOT yet live-eyeballed |
...
> Rule-A status: **P8 SHIPPED** (`44ec1e7`), **P9 SHIPPED** (`883a911`, both 2026-06-16). **P11 (§4.5), P13** still need explicit pros/cons approval before execution. **P10, P12** remain net-new additive shells (still flag-gated; confirm with the user but lower-risk).
...
- **P9 — Geofences drawer + all-geofences ring overlay** ✅ **SHIPPED `883a911`** (§P9; strategic §3.4).
  *Built:* `horizon/GeofencesDrawer.tsx` (right-side drawer mirroring the P8 shell) + `provider/MapLibreGeofenceRings.tsx` + 2 additive `geofenceService` exports. Lists `listUserPlaces` filtered to `geofenceRadiusM != null`; radius edit via stepped presets `[100..5000]` (within the `geofence_radius_m` CHECK 10..50000) / remove via `setPlaceGeofence(id, null)`; recent `geofence_events` with **"mark reviewed"** = the FIRST producer/consumer of the dead `surfaced`/`surfaced_at` (writes BOTH — `surfaced_at` has no default; RLS `geofence_events_update_surfaced` verified live); coral equirectangular ring per place (mirrors `MapLibreAtlasHalos`); honest **"requires Live ON"** banner (detection is broadcast-coupled — `processPosition` only runs from `startLocationBroadcast`'s tick). **No migration** (cols + UPDATE RLS already exist). `completion_geofence` stays a `horizonStubs` stub.
  *Deviations (named):* (1) drawer uses the **P8 right-side shell** (the established Horizon drawer), not the playbook's literal "copy `LiveBroadcastSheet` bottom-sheet." (2) `geofencedPlaces` is owned by **PulseMapView** (single source for both the ring overlay and the drawer list), refetched after a drawer edit — instead of the drawer self-fetching. (3) Entry is a **bottom-left trigger chip** (no shared drawer host exists yet) + a drawer-driven rings-visibility toggle.
...
- **P9 needs a LIVE eyeball + has no real events yet.** `geofence_events` is **empty in prod** (0 rows — geofencing has only ever been a stub), so the drawer's "Recent activity" list + "mark reviewed" path can only be exercised after a real broadcast crosses a geofence (needs Live ON + a geofenced place you physically enter/leave, or a hand-seeded test row). The places list + radius edit + ring overlay ARE exercisable now (8 prod places already have radii). Rings/drawer are tsc + review-verified, not live-eyeballed.
- **P9 rings are equirectangular (not geodesic)** — same approximation as `MapLibreAtlasHalos`; fine at city scale, distorts near the poles. Documented in the overlay header.
...
P0–P9 shipped, `mapHorizon` OFF, P7 live. P8 (Live drawer) = `44ec1e7`, P9 (Geofences drawer + rings) = `883a911` — both tsc-clean + adversarially reviewed but **not yet live-eyeballed** (§5). Resume at **P10** (Routes & planning drawer). Rule-A approvals still needed for P11, P13.
