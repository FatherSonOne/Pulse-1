...
> Rule-A status: **P8, P11 (§4.5), P13** still need explicit pros/cons approval before execution. **P9, P10, P12** are net-new additive shells (still flag-gated; confirm with the user but lower-risk).

- **P8 — Live team first-class drawer** (Build Playbook §P8; strategic §3.3, §4 / Rule-A).
  *Delta:* P7 is DONE, so presence is live — the drawer will actually populate. Compose `useLivePresence` + `startLocationBroadcast` + `setBroadcastRecipients` + consents + `listActiveShares`. Wire the **unsurfaced-but-real** `setLocationSharing` (zero callers today) as the "share my location" master switch. Display + POPULATE `user_locations.location_label` (real reader in `teamRadarService`/`TeamRadarTile`, zero producer). List active ETA shares with the **5-min grace-window** awareness (`get_eta_share_by_token`). Move the broadcast trigger out of `MapFilterBar` into the drawer (keep the pill on the OFF path). Normalize `AcceptedRoute.path` off the `google.maps` type → `provider/types.ts` `LatLngLiteral`.
...
- **Parallel sessions commit to `main`.** Always `git fetch` + check divergence before committing; use explicit-path commits.

---

## 6. One-line state

P0–P7 (minus drawers) shipped + visually verified, `mapHorizon` OFF, P7 live. Resume at **P8** (Live drawer — the P7 payoff). Rule-A approvals still needed for P8, P11, P13.
