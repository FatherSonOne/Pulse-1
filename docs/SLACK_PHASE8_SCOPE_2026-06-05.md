...
| D-E | Send UI surface | **Inline send inside the Focus column** (compose a short DM in place), mirroring the *intent* of the email compose bridge but NOT navigating away. **CONFIRMED via mockup 2026-06-06 — Direction A** (`_design-playground/slack-redesign.html`; A inline chosen over B sidecar / C popover). | Email navigates to its own full section because Email *is* a section; Slack has no section, so a navigate-away has no destination. A small inline composer (textarea + Send) in `FocusColumn` keeps the act local to the contact. | (a) Sidecar (email reply pattern) — covers the detail, no Slack panel to slide into; (b) Command popover — cramped for longer messages; (c) Global modal — heavier than a one-line DM warrants; (d) `slack://` deep-link — abandons in-app send, contradicts handoff. |
