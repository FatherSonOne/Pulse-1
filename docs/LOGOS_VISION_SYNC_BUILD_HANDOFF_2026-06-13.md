...
**v1 (single-tenant) is COMPLETE.** Deferred follow-ups (not blockers): P6 relationship-score re-weight + `logos_case_state` mirror/poller; a dedicated `ai-router` task for clean P5 metering; **multi-tenant graduation** — when writes go beyond one shared owner key, route through the two-header ecosystem bridge (not a client-embedded service key), then the public marketing panel + corrected copy can be re-enabled. Live acceptance (the in-UI flows: link a contact, note→case-log, email→activity, AI suggest, case-state display) is pending an owner pass with `logosVisionSync` ON.
