...
- **Last issue worked:** [#106](https://github.com/FatherSonOne/Pulse-1/issues/106) — Post-meeting AI. **DONE + closed** (`06f3554`). Investigation found the issue's premise partly outdated: **native meeting AI is already REAL** — Deepgram live transcription + ai-router `generateSummary` (action-items/decisions/keyPoints) on Pulse-hosted meetings → `pulse_video_rooms` (acceptance #1 already met, untouched). The actual dishonest surface was **Meeting Analytics**: `AnalyticsModal` + the `MeetingsRailKicker` dashboard stats read the `meetings` table for counts/sentiment/topics/decisions, but **nothing in the repo writes that table** (every `.from('meetings')` is a SELECT; native→`pulse_video_rooms`; Entomate's `meeting.processed` posts a bot recap *message*, not analytics rows) — so it was permanently empty and its empty-state copy ("Start a Pulse meeting with AI Scribe to populate analytics") was actively false. **Operator decision: gate on Entomate.** Gated both analytics surfaces on `isEntomateConnected()` with honest empty/locked states; removed the false copy; hid the dead rail-kicker zeros when not connected (acceptance #2 met). Filed **#121** for the deeper write-back gap (analytics won't populate even connected until a writer exists).
- **Last run (date):** 2026-05-26 — #106 (closed) + #121 (analytics write-back follow-up) opened. P1 truth-in-product sweep: #105 ✅ + #106 ✅; only #107 left before the #104 umbrella can close.
- **Next up:** [#107](https://github.com/FatherSonOne/Pulse-1/issues/107) — **Relabel 'contact enrichment' (it's internal dedup, not third-party enrichment)** (P1, `priority: medium`, unblocked). The likely target is `contactEnrichmentService.ts` — it's signature-parse + dedup, not an external data source, so any UI/labels calling it "enrichment" overclaim. Cheap truth-in-product relabel (mirror the honesty pattern from #105/#106). **Closing #107 lets the #104 umbrella close** (+ a #115 QA click-through). *Why #107 over the nominally-higher `priority: high` compliance issues:* #109 (SMS 10DLC/TCPA) gates **real SMS, which is hidden for v1** (#100) → not launch-blocking; the v1-relevant high-priority compliance is #111 (GDPR/CCPA DSAR + residency) and #112 (ToS/Privacy/moderation), which should follow #107. Several compliance issues (#110 CALEA, #112, #113 E2EE) are **decisions, not code** — draft a recommendation doc + ask, don't guess.
...
- **Notes for next run:** Re-read the Status Table first. P0 clear of code work; #100/#101/#102/#103/#105/#106 done. **Do NOT re-pick #104** (open umbrella; #105 ✅ + #106 ✅ — only #107 + a #115 QA pass left before it closes). Take **#107 (relabel 'contact enrichment', P1 medium)** — picked over the higher-`priority` compliance issues because #109 gates v1-hidden SMS (not launch-blocking); the real v1 compliance is #111/#112 *after* #107. Likely target: `contactEnrichmentService.ts` (signature-parse + dedup, not third-party data) + wherever the UI labels it "enrichment." Cheap relabel; mirror the #105/#106 honesty pattern. Gating/labeling toolkit: `featureFlags.ts` (`inAppSms`/`emailCampaigns` examples) for static flags, or the runtime `isEntomateConnected()` gate pattern used in #106. For the compliance reads (#110/#112/#113), several are **decisions, not code** — draft + ask, don't guess. Doc-tracking note: `docs/*.md` is **gitignored** but `docs/*_RUNBOOK.md` / `*_ROADMAP.md` / `*_GUIDE.md` / `*_HANDOFF_*` / `*_AUDIT_*` are allowlisted (this roadmap, both runbooks, and the audit track normally); other doc names need `git add -f`.
