...
- **Last issue worked:** [#101](https://github.com/FatherSonOne/Pulse-1/issues/101) — push **DONE**: `send-push` deployed + secrets set, verified live (`sent:1` to a real FCM subscription for jehovahsneaky83@gmail.com).
- **Last run (date):** 2026-05-26 — #101 push end-to-end. Beyond the sender: disabled gateway `verify_jwt` (`2520c05`), reconciled all 4 disconnected notification-enable paths to actually `subscribe()` (`06cb825`/`b0303ab`/`7d012c6`), injected the push handler into the VitePWA-generated SW via `workbox.importScripts` (`a77bc40`), added an always-visible Enable/Disable push card (`619ef86`/`61a2dc9`), pinned `vite preview` to 5173. Human set VAPID + `PUSH_DISPATCH_SECRET` secrets on pulse-chat.
