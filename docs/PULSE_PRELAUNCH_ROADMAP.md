...
| [#102](https://github.com/FatherSonOne/Pulse-1/issues/102) | Transactional email deliverability — verified domain | critical | — | blocked (DMARC + inbox test) | `ec73a65` — all senders aligned to verified `pulse.logosvision.org` (`send-security-alert` was `onboarding@resend.dev` owner-only; `check-search-alerts` defaulted to unowned `pulse.ai`; invites used parent domain). **Deployed** to `pulse-chat`: send-email v15, send-security-alert v34, check-search-alerts v26 (`5b190cc` declared their `verify_jwt=false` first so the deploy didn't flip them). **Remaining (human):** add DMARC TXT at IONOS + run external-inbox delivery test. Runbook: `docs/EMAIL_DELIVERABILITY_RUNBOOK.md` |
...
- **Open blockers / decisions waiting on the human:**
  - **#102 email — code DONE + deployed, blocked on human:** the 3 edge functions are deployed to `pulse-chat` (send-email v15 / send-security-alert v34 / check-search-alerts v26, all `verify_jwt=false` preserved). Remaining: (a) add the DMARC TXT record at IONOS (`_dmarc.pulse.logosvision.org`, value `v=DMARC1; p=none; rua=mailto:support@logosvision.org; pct=100; aspf=r; adkim=r`); (b) run the external-inbox delivery test (invite + security alert + search alert → a non-owner Gmail, confirm receipt + SPF/DKIM/DMARC pass). Steps in `docs/EMAIL_DELIVERABILITY_RUNBOOK.md` §2 + §4. Live curl test of send-security-alert needs `CRON_SECRET` + a target address.
