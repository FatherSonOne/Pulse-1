...
| [#102](https://github.com/FatherSonOne/Pulse-1/issues/102) | Transactional email deliverability — verified domain | critical | — | blocked (human DNS+deploy+test) | `ec73a65` — all senders aligned to verified `pulse.logosvision.org` (`send-security-alert` was `onboarding@resend.dev` owner-only; `check-search-alerts` defaulted to unowned `pulse.ai`; invites used parent domain). Code done. **Remaining (human):** add DMARC TXT at IONOS, deploy 3 edge fns, run external-inbox test. Runbook: `docs/EMAIL_DELIVERABILITY_RUNBOOK.md` |
...
- **Open blockers / decisions waiting on the human:**
  - **#102 email — code DONE, blocked on human:** (a) add the DMARC TXT record at IONOS (`_dmarc.pulse.logosvision.org`, start `p=none`); (b) deploy the 3 changed edge functions (`send-security-alert`, `check-search-alerts`, `send-email`) to `pulse-chat`; (c) run the external-inbox delivery test. All copy-paste steps in `docs/EMAIL_DELIVERABILITY_RUNBOOK.md`.
