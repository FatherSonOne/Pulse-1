...
| [#102](https://github.com/FatherSonOne/Pulse-1/issues/102) | Transactional email deliverability — verified domain | critical | — | blocked (stale client key + DMARC) | `ec73a65` aligned all senders to `pulse.logosvision.org`; deployed to `pulse-chat` (send-email v15 / send-security-alert v34 / check-search-alerts v26, `verify_jwt=false` preserved via `5b190cc`). **Live test revealed the real bug:** server-side email is FINE (edge fns use the `fm1@qntmecos.com` Resend account where the domain IS verified), but `.env` `VITE_RESEND_API_KEY` is a **stale key from the abandoned `jehovahsneaky83` account (no verified domain)** → client-side `inviteService`/`rsvpService` 403. **Remaining:** (1) repoint `VITE_RESEND_API_KEY` to an `fm1@qntmecos.com` send-only key (owner; better: route both through the `send-email` edge fn in #104); (2) add DMARC TXT on the `fm1` domain; (3) external-inbox test. Runbook: `docs/EMAIL_DELIVERABILITY_RUNBOOK.md` |
...
- **Last issue worked:** [#102](https://github.com/FatherSonOne/Pulse-1/issues/102) — transactional email deliverability. Code **DONE** (`ec73a65`): every sender now uses the Resend-verified `pulse.logosvision.org`. Marked `status: blocked` — the two remaining acceptance criteria (DMARC DNS record + external-inbox delivery test) are human/DNS/deploy actions.
...
- **Open blockers / decisions waiting on the human:**
  - **#102 email — code DONE + deployed; live test exposed a stale-key bug.** Edge fns deployed to `pulse-chat` and use the **`fm1@qntmecos.com`** Resend account where the domain is verified → server-side email works. But `.env` `VITE_RESEND_API_KEY` is a **stale key from the abandoned `jehovahsneaky83` Resend account (zero verified domains)** — confirmed via 403s (subdomain, parent, and sandbox-owner = jehovahsneaky83). So client-side `inviteService`/`rsvpService` are broken. Remaining: (a) **repoint `VITE_RESEND_API_KEY`** (in `.env` + Vercel) to a send-only key from the `fm1@qntmecos.com` account — owner action, `.env` gitignored; (b) **proper fix in #104:** route invite/RSVP through the `send-email` edge fn (kills the browser key exposure + stale-key drift); (c) add the **DMARC** TXT record on the `fm1` domain (`_dmarc.pulse.logosvision.org`, `v=DMARC1; p=none; rua=mailto:support@logosvision.org; pct=100; aspf=r; adkim=r`); (d) external-inbox delivery test. To send a test to `fm1@qntmecos.com` right now needs an `fm1`-account send-only key OR a fresh JWT to hit `send-email` (e2e fixture token is expired).
