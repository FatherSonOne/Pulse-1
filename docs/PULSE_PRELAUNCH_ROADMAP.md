...
- **Last issue worked:** [#105](https://github.com/FatherSonOne/Pulse-1/issues/105) — Email campaigns. **DONE + closed** (`0767141`). Operator decision: **hide** for v1 (feature likely cut). Added `emailCampaigns` flag (OFF) mirroring `inAppSms`; hid the email sidebar "Tools" section (Campaigns was its only item), gated the `currentView === 'campaigns'` render, and added a reset effect → the unsafe per-recipient `emailCampaignService.send()` loop is unreachable. Also this run, per operator decisions on the #104 audit's open questions: **wired MESSAGE_ANALYTICS into nav** (`3d51235`, Intelligence section, real deployed schema) and left **Map** in Experimental ("don't abandon, circle back"). #104 audit doc updated with the resolutions (`f0e5c74`).
- **Last run (date):** 2026-05-26 — #105 (closed) + #104 decision follow-through (MESSAGE_ANALYTICS wired, Map decision recorded). P1 truth-in-product execution underway.
- **Next up:** [#106](https://github.com/FatherSonOne/Pulse-1/issues/106) — **Post-meeting AI: own the extraction in Pulse or relabel the Entomate handoff** (P1, `priority: high`, unblocked). Evidence from the #104 audit: the Meetings surface exports to / hands off post-meeting AI to Entomate (`Meetings.tsx:831-841`, `MeetingsComponents.tsx:931-933,1033-1107`); the "Export" button is conditional on `entomateConnected` (`MeetingsComponents.tsx:895,1033`). **The truth-in-product question:** does the UI currently *imply* Pulse does the meeting AI itself (overclaim), or is the Entomate handoff already labeled honestly? Likely a **relabel + honest-empty-state** task, possibly a decision (own-it-in-Pulse is a big build vs. relabel-the-handoff is cheap). Inspect those files first; if it's a "build vs. relabel" judgment call, draft the recommendation and ask. After #106: #107 (enrichment relabel — `contactEnrichmentService.ts`, it's signature-parse + dedup not third-party enrichment), then compliance reads #109/#111/#112 (several are **decisions, not code** — draft + ask).
...
- **Notes for next run:** Re-read the Status Table first. P0 clear of code work; #100/#101/#102/#103/#105 done. **Do NOT re-pick #104** (open umbrella; its 3 product calls are all resolved — closes with #106/#107 + a #115 QA pass). Take **#106 (post-meeting AI, P1 high)**. The #104 audit located the Entomate handoff: `Meetings.tsx:831-841`, `MeetingsComponents.tsx:931-933,1033-1107`, Export gated on `entomateConnected` (`MeetingsComponents.tsx:895,1033`). First read those to judge whether the UI overclaims that Pulse does meeting AI itself, or already labels the Entomate handoff honestly. Likely a **relabel** (cheap, truth-in-product) rather than owning the extraction in Pulse (big build). If it's a build-vs-relabel judgment, draft a short recommendation + ask. The established gating/labeling tools: the `featureFlags.ts` mechanism + the `inAppSms`/`emailCampaigns` worked examples. Doc-tracking note: `docs/*.md` is **gitignored** but `docs/*_RUNBOOK.md` / `*_ROADMAP.md` / `*_GUIDE.md` / `*_HANDOFF_*` / `*_AUDIT_*` are allowlisted (this roadmap, both runbooks, and the audit track normally); other doc names need `git add -f`.
