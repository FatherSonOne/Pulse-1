...
- **Last issue worked:** [#104](https://github.com/FatherSonOne/Pulse-1/issues/104) — Feature-flag audit (P1 high). **Audit DONE** (`72c7571`, `docs/FEATURE_FLAG_AUDIT_v1.md`), issue **kept open** as the truth-in-product umbrella. Evidence-based inventory of all ~18 `AppView` surfaces: the **only** hard mock in `src/` is `smsService.isMockMode()=>true` (already gated via `inAppSms` #100), so **no orphan stub needed gating** — a valid "gate nothing" outcome. Every other non-real surface is owned by a dedicated issue (#105 email campaigns, #106 post-meeting AI, #107 enrichment relabel, #108 CRM pagination). Push is REAL (#101). Acceptance #3 (click-through) is a human QA pass → #115. Also this run: **closed #102** (operator set the DMARC record — its last acceptance item).
- **Last run (date):** 2026-05-26 — #104 audit (kept open) + closed #102. **P0 tier now fully clear of code work** (#99 in-progress = human live-smoke + deferred CRM refactor; #100/#101/#102/#103 done). Moving into P1 execution.
- **Next up:** [#105](https://github.com/FatherSonOne/Pulse-1/issues/105) — **Email campaigns: flag for v1 (or add batching/rate-limit/unsubscribe)** (P1, `priority: high`, unblocked). The #104 audit pinned the exact concern: the Campaigns dashboard/builder is reachable from the Email surface (`PulseEmailClientRedesign.tsx:551-555`) but `emailCampaignService.send()` is a naive per-recipient loop (`emailCampaignService.ts:148-204`) → spam-flag/deliverability risk. **Product call the audit surfaced:** does #105 hide the *entire* Campaigns entry behind a flag for v1 (fastest truth-in-product path — add an `emailCampaigns` flag OFF following the `inAppSms` pattern), or harden the loop (batching + rate-limit + unsubscribe + List-Unsubscribe header)? Given v1 = "ship only what's real", the flag-off path is likely right unless the send loop can be made compliant quickly — **draft the recommendation and ask the human if non-obvious.** After #105: #106 (post-meeting AI relabel/own), #107 (enrichment relabel), then the compliance reads #109/#111/#112 (several are **decisions, not code** — draft + ask).
- **Open blockers / decisions waiting on the human:**
  ...
  - **#104 product calls (3, surfaced by the audit):** (a) does **#105** hide the entire Email→Campaigns entry behind a flag for v1, or only harden the send loop? (b) is the "Experimental" nav-section labeling enough for the Map surface in v1, or flag-hide until the cluster refactor lands? (c) `MESSAGE_ANALYTICS` is a real-but-orphaned route (in `App.tsx` render switch, absent from nav) — wire into nav or delete the dead route? None blocks #105; (a) should be answered *within* #105.
...
- **Notes for next run:** Re-read the Status Table first. P0 is clear of code work (#99 = human smoke + deferred CRM refactor; #100/#101/#102/#103 done). #104 audit done — **do NOT re-pick #104**; it's the open umbrella that closes with #105/#106/#107 + a #115 QA pass. Take **#105 (email campaigns, P1 high)**. The #104 audit already located everything: campaigns reachable at `PulseEmailClientRedesign.tsx:551-555`, unsafe send loop at `emailCampaignService.ts:148-204`. Decide flag-off-for-v1 vs. harden-the-loop — per "ship only what's real," flagging the Campaigns entry OFF behind a new `emailCampaigns` flag (mirror the `inAppSms` pattern: config entry OFF + Sidebar nav-hide + `App.tsx` guard, or hide the campaigns tab within `PulseEmailClientRedesign`) is the fast honest path; only harden the loop if it can be made compliant (batching + rate-limit + unsubscribe + `List-Unsubscribe` header) without a big lift. If the call is non-obvious, draft a short recommendation and ask the human. Read `docs/FEATURE_FLAG_AUDIT_v1.md` for the surface map + the flag pattern. Doc-tracking note: `docs/*.md` is **gitignored** but `docs/*_RUNBOOK.md` / `*_ROADMAP.md` / `*_GUIDE.md` / `*_HANDOFF_*` / `*_AUDIT_*` are allowlisted (this roadmap, both runbooks, and the audit track normally); other doc names need `git add -f`.
