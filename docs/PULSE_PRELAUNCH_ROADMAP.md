...
| [#102](https://github.com/FatherSonOne/Pulse-1/issues/102) | Transactional email deliverability — verified domain | critical | — | blocked (DMARC + client key swap) | `ec73a65` aligned all senders to `pulse.logosvision.org`; deployed to `pulse-chat` (send-email v15 / send-security-alert v34 / check-search-alerts v26, `verify_jwt=false` preserved via `5b190cc`). **Delivery test PASSED:** live send `noreply@pulse.logosvision.org`→`fm1@qntmecos.com` returned 200 (id `f314ac89…`) on the `fm1@qntmecos.com` account — domain verified, server-side senders good. The only residual is **client-side:** `.env`/Vercel `VITE_RESEND_API_KEY` is a stale key from the abandoned `jehovahsneaky83` account → `inviteService`/`rsvpService` 403. **Remaining (human):** (1) repoint `VITE_RESEND_API_KEY` in `.env` + **Vercel** to the `fm1` send-only key (proper: route through `send-email` edge fn in #104); (2) add DMARC TXT on the `fm1` domain. Runbook: `docs/EMAIL_DELIVERABILITY_RUNBOOK.md` |
