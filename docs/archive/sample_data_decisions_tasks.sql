-- Sample Data for AI-Enhanced Decisions & Tasks Demo
...
INSERT INTO decisions (
  ...
  'We need to decide between GitHub Actions, GitLab CI, or Jenkins for our deployment pipeline. Consider cost, ease of use, and integration with existing tools.',
  'voting',
  'technical',
  (SELECT id FROM auth.users LIMIT 1),
  NOW() - INTERVAL '2 days', -- Created 2 days ago (stale!)
  NOW() - INTERVAL '2 days',
  '{}'::jsonb
) ON CONFLICT DO NOTHING;

-- Decision 2: Recent voting (active)
INSERT INTO decisions (
  id,
  workspace_id,
  title,
  ...
  'Should we break down our monolith into microservices? This would improve scalability but increase complexity.',
  'voting',
  'technical',
  (SELECT id FROM auth.users LIMIT 1),
...
INSERT INTO decisions (
  ...
  'Convert our codebase from JavaScript to TypeScript for better type safety and developer experience.',
  'decided',
  'technical',
  (SELECT id FROM auth.users LIMIT 1),
  NOW() - INTERVAL '5 days',
  NOW() - INTERVAL '1 day',
  NOW() - INTERVAL '1 day',
  'Approved - will migrate incrementally over next 2 months',
  '{}'::jsonb,
  'medium'
) ON CONFLICT DO NOTHING;
...
INSERT INTO decisions (
  ...
  'Users have requested dark mode. Should we prioritize this for Q2?',
  'voting',
  'product',
  (SELECT id FROM auth.users LIMIT 1),
...
-- Task 1: Overdue and urgent (should trigger urgent nudge!)
INSERT INTO tasks (
  id,
  workspace_id,
  title,
  ...
  NOW() - INTERVAL '1 hour',
  '{}'::jsonb,
  95.0 -- Very high AI priority
) ON CONFLICT DO NOTHING;

-- Task 2: Due today (important)
INSERT INTO tasks (
  id,
  workspace_id,
  title,
...
-- ============================================
-- SAMPLE VOTES (for participation metrics)
-- ============================================

-- Add some votes to decisions
...
SELECT
  ...
  NOW() - INTERVAL '1 day'
FROM decisions d
WHERE d.title = 'Switch to TypeScript'
ON CONFLICT DO NOTHING;
...
FROM decisions d
WHERE d.title = 'Migrate to Microservices Architecture'
ON CONFLICT DO NOTHING;
...
SELECT
  ...
  decision_type,
  ai_risk_level,
  DATE_PART('day', NOW() - created_at::timestamp) || ' days ago' as created
FROM decisions
...
SELECT
  ...
  ai_priority_score,
  CASE
    WHEN due_date < NOW() AND status != 'done' THEN '⚠️ OVERDUE'
