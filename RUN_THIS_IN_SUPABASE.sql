...
-- ================================================
-- This will create the security monitoring tables
-- Go to: https://supabase.com/dashboard/project/ucaeuszgoihoyrvhewxk/sql/new
...
CREATE INDEX IF NOT EXISTS idx_activity_logs_severity ON public.activity_logs(severity) WHERE severity IN ('warning', 'critical');
CREATE INDEX IF NOT EXISTS idx_activity_logs_user_date ON public.activity_logs(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_activity_logs_composite ON public.activity_logs(user_id, action_category, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_security_alerts_alert_level ON public.security_alerts(alert_level);
CREATE INDEX IF NOT EXISTS idx_security_alerts_status ON public.security_alerts(status);
CREATE INDEX IF NOT EXISTS idx_security_alerts_unread ON public.security_alerts(user_id, created_at DESC) WHERE status = 'new';
CREATE INDEX IF NOT EXISTS idx_security_alerts_composite ON public.security_alerts(user_id, status, alert_level, created_at DESC);
...
SELECT 'Migration Complete!' as status,
       'Tables Created: activity_logs, security_alerts, security_settings' as result;
