...
import {
  ...
  Bot, Zap, Link2, Activity, ToggleLeft, ToggleRight, ChevronDown, ChevronUp,
} from 'lucide-react';
...
export const IntegrationsSettings: React.FC<IntegrationsSettingsProps> = ({ user, userId }) => {
  ...
  const [newAppName, setNewAppName] = useState<'entomate' | 'logos_vision'>('entomate');
  ...
  /** Generate a cryptographically random 64-char hex token (browser-side) */
  ...
  const setupWorkspaceBot = async (workspaceId: string) => {
    ...
    try {
      ...
      setEcosystemSetupStatus(`Created: ${result.created.join(', ') || 'none'} | Already existed: ${result.existing.join(', ') || 'none'}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-8 animate-slide-up">
      ...
      {/* ═══════════════════════════════════════════════════════════
          ECOSYSTEM BRIDGE — Logos Vision × Pulse × Entomate
          ═══════════════════════════════════════════════════════════ */}
      <div className="mt-8 border border-violet-500/20 rounded-2xl overflow-hidden bg-violet-500/5">
        {/* Header */}
        <button
          type="button"
          className="w-full flex items-center justify-between px-6 py-4 hover:bg-violet-500/10 transition"
          onClick={() => setEcosystemExpanded(e => !e)}
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <div className="text-xs text-zinc-400">Connect Logos Vision, Pulse & Entomate</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {ecosystemLoading && <Loader2 className="w-4 h-4 text-violet-400 animate-spin" />}
            <span className="text-xs text-zinc-500">{ecosystemConfigs.filter(c => c.enabled).length}/{ecosystemConfigs.length} active</span>
            {ecosystemExpanded ? <ChevronUp className="w-4 h-4 text-zinc-400" /> : <ChevronDown className="w-4 h-4 text-zinc-400" />}
          </div>
        </button>

        {ecosystemExpanded && (
          <div className="px-6 pb-6 space-y-5 border-t border-violet-500/15">

            {/* Connected Apps */}
            {ecosystemConfigs.length > 0 && (
              <div className="mt-5 space-y-3">
                <div className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Connected Apps</div>
                {ecosystemConfigs.map(config => {
                  const testStatus = ecosystemTestStatus[config.app_name];
                  const appEmoji = config.app_name === 'entomate' ? '🤖' : config.app_name === 'logos_vision' ? '🔮' : '🔗';
                  return (
                    <div key={config.id} className="rounded-xl bg-white/[0.03] border border-white/[0.07] overflow-hidden">
                      {/* App row */}
                      <div className="flex items-center gap-3 p-3">
                        <span className="text-xl">{appEmoji}</span>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-white capitalize">
                              {config.app_name.replace('_', ' ')}
                            </span>
                            <span className={`w-2 h-2 rounded-full ${config.enabled ? 'bg-emerald-500' : 'bg-zinc-600'}`} />
                          </div>
                          <div className="text-xs text-zinc-500 truncate">{config.api_url}</div>
                          {config.last_heartbeat && (
                            <div className="text-xs text-zinc-600 mt-0.5">
                              Last heartbeat: {new Date(config.last_heartbeat).toLocaleString()}
                            </div>
                          )}
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          <button
                            ...
                            className="px-2 py-1 rounded-lg text-xs bg-white/[0.05] hover:bg-white/[0.10] text-zinc-400 hover:text-white border border-white/[0.08] transition"
                            title="View tokens"
                          >
                            Tokens
                          </button>
                          ...
                          <button
                            ...
                            className="px-2 py-1 rounded-lg text-xs bg-white/[0.05] hover:bg-white/[0.10] text-zinc-400 hover:text-white border border-white/[0.08] transition flex items-center gap-1"
                          ...
                          <button
                            type="button"
                            onClick={() => toggleEcosystemApp(config)}
                      ...
                      {expandedConfigTokens === config.id && (
                        <div className="border-t border-white/[0.06] px-4 py-3 bg-black/20 space-y-2">
                          <p className="text-[10px] text-zinc-500 mb-2">
                            Copy these into {config.app_name.replace('_', ' ')}'s <code>ecosystem_config</code> table (values are <strong>swapped</strong>).
                          </p>
                          {[
                            { label: 'Service Token (Pulse→them)', value: config.service_token, hint: `→ paste as inbound_token in ${config.app_name}`, key: `svc-${config.id}` },
                            { label: 'Inbound Token (them→Pulse)', value: config.inbound_token, hint: `→ paste as service_token in ${config.app_name}`, key: `inb-${config.id}` },
                            ...
                            <div key={key}>
                              <div className="flex items-center justify-between mb-0.5">
                                ...
                                <button
                                  ...
                                  className="text-[10px] px-2 py-0.5 rounded bg-white/[0.06] hover:bg-white/[0.10] text-zinc-400 border border-white/[0.08] transition"
                                >
                                  {copiedToken === key ? '✓ Copied' : 'Copy'}
                                </button>
                              </div>
                              <div className="font-mono text-[10px] text-zinc-400 bg-black/30 rounded px-2 py-1.5 truncate border border-white/[0.04]">
                                {value}
                              </div>
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.07] p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={newAppName}
                    onChange={(e) => setNewAppName(e.target.value as any)}
                    title="Select app to connect"
                    aria-label="Select app to connect"
                    className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white focus:outline-none focus:border-violet-500 transition"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="https://app.entomate.com/api"
                    className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-violet-500 transition"
                  />
                </div>
              </div>

              {/* Token fields with Generate buttons */}
              <div className="space-y-3">
                <div className="rounded-lg bg-violet-500/10 border border-violet-500/20 p-3 text-xs text-zinc-400 leading-relaxed">
                  <span className="text-violet-300 font-semibold block mb-1">How tokens work:</span>
                  <span className="font-semibold text-zinc-300">Service token</span> = what Pulse sends when calling <em>their</em> API. Paste this into the other app's <code className="text-violet-300">inbound_token</code>.<br />
                  <span className="font-semibold text-zinc-300">Inbound token</span> = what they send when calling Pulse. Paste this into the other app's <code className="text-violet-300">service_token</code>.
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex gap-1">
                      <button type="button" onClick={() => setNewServiceToken(generateToken())}
                        className="text-[10px] px-2 py-0.5 rounded bg-violet-500/20 hover:bg-violet-500/30 text-violet-300 border border-violet-500/25 transition">
                        Generate
                      </button>
                      {newServiceToken && (
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-xs text-zinc-300 font-mono placeholder-zinc-600 focus:outline-none focus:border-violet-500 transition"
                  />
                  {newServiceToken && (
                    <p className="text-[10px] text-zinc-600 mt-1">
                      → Paste this value as <code>inbound_token</code> in {newAppName.replace('_', ' ')}'s ecosystem_config
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {newInboundToken && (
                    <p className="text-[10px] text-zinc-600 mt-1">
                      → Paste this value as <code>service_token</code> in {newAppName.replace('_', ' ')}'s ecosystem_config
                    </p>
                  )}
                </div>
              </div>
            ...
            {ecosystemConfigs.some(c => c.app_name === 'entomate' && c.enabled) && (
              <div className="rounded-xl bg-violet-500/10 border border-violet-500/20 p-4 space-y-3">
                ...
                <p className="text-xs text-zinc-400">
                  Creates <code className="text-violet-300">#entomate-meetings</code>, <code className="text-violet-300">#entomate-tasks</code>, <code className="text-violet-300">#entomate-alerts</code> in the default workspace.
                </p>
                <button
                  type="button"
                  onClick={() => setupWorkspaceBot(userId)}
                  className="px-4 py-2 rounded-lg text-xs font-semibold bg-violet-500/20 hover:bg-violet-500/30 text-violet-300 border border-violet-500/30 transition flex items-center gap-2"
                >
                  <Bot className="w-3 h-3" />
                  Setup Entomate Channels
                </button>
                {ecosystemSetupStatus && (
                  <p className="text-xs text-zinc-400 font-mono">{ecosystemSetupStatus}</p>
                )}
              </div>
            )}
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-between text-xs font-bold text-zinc-400 uppercase tracking-wider hover:text-zinc-200 transition"
              >
              ...
              {ecosystemEventsExpanded && (
                <div className="mt-2 rounded-xl bg-black/30 border border-white/[0.06] overflow-hidden">
                  {ecosystemEvents.length === 0 ? (
                    <div className="text-xs text-zinc-500 text-center py-4">No events yet</div>
                  ) : (
                      ...
                      <tbody>
                        {ecosystemEvents.map(ev => (
                          <tr key={ev.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                            <td className="px-3 py-2 text-zinc-300 font-mono">{ev.event_type}</td>
                            <td className="px-3 py-2 text-zinc-500 capitalize">{ev.source}</td>
                            <td className="px-3 py-2">
                              <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${
                                ev.status === 'processed' ? 'bg-emerald-500/20 text-emerald-400' :
                                ev.status === 'failed' ? 'bg-rose-500/20 text-rose-400' :
