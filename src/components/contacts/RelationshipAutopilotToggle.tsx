    ...
    <div className={`flex items-center justify-between p-3 rounded-xl border transition-colors ${
      ...
      <div className="flex items-center gap-2 min-w-0">
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
          localEnabled ? 'bg-indigo-100 dark:bg-indigo-900/40' : 'bg-zinc-200 dark:bg-zinc-700'
        }`}>
          <i className={`fa-solid fa-robot text-xs ${
            localEnabled ? 'text-indigo-600 dark:text-indigo-400' : 'text-zinc-500 dark:text-zinc-400'
          }`} />
        </div>
        <div className="min-w-0">
          <p className={`text-xs font-semibold truncate ${
            localEnabled ? 'text-indigo-800 dark:text-indigo-200' : 'text-zinc-700 dark:text-zinc-300'
          }`}>
            Autopilot {localEnabled ? 'on' : 'off'}
          </p>
          <p className={`text-[10px] truncate ${
            localEnabled ? 'text-indigo-500 dark:text-indigo-400' : 'text-zinc-400 dark:text-zinc-500'
          }`}>
      ...
      <button
        ...
        className={`relative flex-shrink-0 rounded-full transition-colors ${
          localEnabled ? 'bg-indigo-500' : 'bg-zinc-300 dark:bg-zinc-600'
        }`}
